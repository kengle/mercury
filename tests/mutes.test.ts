...
describe("MuteService", () => {
  test("two-step confirm: first call returns warning", () => {
    const result = svc.create({ userId: "user1", duration: "10m" }, "admin");
    expect(result.warning).toBeDefined();
    expect(svc.isMuted("user1")).toBe(false);
  });

  test("two-step confirm: second call with confirm mutes", () => {
    const result = svc.create({ userId: "user1", duration: "10m", confirm: true }, "admin");
    expect(result.warning).toBeUndefined();
    expect(svc.isMuted("user1")).toBe(true);
  });
  ...
  test("unmute removes mute", () => {
    svc.create({ userId: "user1", duration: "10m", confirm: true }, "admin");
  ...
  test("list mutes", () => {
    svc.create({ userId: "user1", duration: "10m", confirm: true }, "admin");
    svc.create({ userId: "user2", duration: "1h", confirm: true }, "admin");
    const all = svc.list();
    expect(all).toHaveLength(2);
  });

  test("duration parsing: minutes", () => {
    svc.create({ userId: "user1", duration: "5m", confirm: true }, "admin");
    const mutes = svc.list();
  ...
  test("duration parsing: hours", () => {
    svc.create({ userId: "user1", duration: "2h", confirm: true }, "admin");
    const mute = svc.list()[0];
    const expectedMs = 2 * 60 * 60 * 1000;
    expect(mute.expiresAt - mute.createdAt).toBeCloseTo(expectedMs, -100);
  });

  test("duration parsing: days", () => {
    svc.create({ userId: "user1", duration: "1d", confirm: true }, "admin");
    const mute = svc.list()[0];
  ...
  test("invalid duration throws", () => {
    expect(() => svc.create({ userId: "user1", duration: "abc", confirm: true }, "admin")).toThrow();
  });

  test("purgeExpiredMutes removes expired entries", () => {
    db.run(
      "INSERT INTO mutes(user_id, expires_at, muted_by, created_at) VALUES (?, ?, ?, ?)",
      ["user1", Date.now() - 1000, "admin", Date.now() - 60000],
    );
    expect(svc.isMuted("user1")).toBe(false);
    svc.purgeExpired();
