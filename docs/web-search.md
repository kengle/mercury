...
Clawbber agents can search the web and fetch content from URLs using `agent-browser`, a headless browser CLI. No API keys required.
...
Clawbber uses browser-based search by default for zero-config operation.
...
The agent receives these instructions via `.clawbber/global/AGENTS.md`:
