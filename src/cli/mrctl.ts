...
if (!API_URL) {
  process.stderr.write("error: API_URL not set\n");
  process.exit(1);
}
if (!CALLER_ID) {
  process.stderr.write("error: CALLER_ID not set\n");
  process.exit(1);
}
...
if (!WORKSPACE_ID) {
  process.stderr.write("error: WORKSPACE_ID not set\n");
  process.exit(1);
}

const headers: Record<string, string> = {
  "x-mercury-caller": CALLER_ID,
  "x-mercury-workspace": WORKSPACE_ID,
...
program
  .name("mrctl")
  .description("Manage Mercury from inside the agent container");

program
  .command("whoami")
  .description("Show caller info and permissions")
  .action(async () => {
...
tasksCmd
  ...
  .action(async (opts) => {
    if (!opts.cron && !opts.at) {
      process.stderr.write("error: Must specify --cron or --at\n");
      process.exit(1);
    }
    print(
...
configCmd
  ...
  .action(async (key?: string) => {
    ...
    if (key) {
      ...
      if (value === undefined) {
        process.stderr.write(`error: Config key not set: ${key}\n`);
        process.exit(1);
      }
...
permsCmd
  .command("set <role> <permissions>")
  .description("Set role permissions (comma-separated)")
  .action(async (role, permsStr) => {
...
program
  .command("stop")
  .description("Abort current run and clear queue")
  .action(async () => {
...
program
  .command("compact")
  .description("Reset session (fresh context)")
  .action(async () => {
